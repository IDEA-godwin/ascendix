...
export default function ViewAllPage({
  ...
  return (
    <main className="text-white w-full h-full overflow-x-hidden" >
      <Header />
      <div className="container mt-[20%] md:mt-[10%] mb-6 py-6 md:py-8 md:px-16 gap-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 text-gray-100 ">
        {Array.from({ length: 3 }).map((_, index) => (
          <ListTile key={index + 1} questId={index.toString()} />
        ))}
        <div className="w-full bg-white py-14 shadow-2xl rounded-md mb-16">
          <Modal>
            <div  className="w-full h-full flex justify-center items-center text-lg font-semibold  group">
